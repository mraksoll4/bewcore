...
configuration options and toggled while bewcored is running with the `logging`
RPC.  For instance, launching bewcored with `-debug` or `-debug=1` will turn on
...
with "play bewcores" on a test network.
...
i.e. `CPPFLAGS="-DDEBUG_LOCKCONTENTION"`, then build and run bewcored.

You can then use the `-debug=lock` configuration option at bewcored startup or
`bewcore-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `bewcore-cli logging [] '["lock"]'`.
...
$ valgrind -v --leak-check=full src/bewcored -printtoconsole
...
To profile a running bewcored process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep bewcored` -- sleep 60
```
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `bewcore-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `bewcore-cli` and the GUI debug console use this table to determine how to
