# User-space, Statically Defined Tracing (USDT) for Bewcore Core

Bewcore Core includes statically defined tracepoints to allow for more
...
**Note**: Bewcore Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Bewcore Core
...
To list probes in Bewcore Core, use `info probes` in `gdb`:
...
The `readelf` tool can be used to display the USDT tracepoints in Bewcore Core.
